...
def utl_tree(n, seed=42):
    ...
    edge1 = np.zeros(2 * n - 3, dtype=int)
    edge2 = np.zeros(2 * n - 3, dtype=int)
    intnod = n - 1
    e = 0
    edge1[e] = 0
    edge2[e] = 1
    np.random.seed(seed)
    for ternod in range(n)[2:]: # iterate over all terminal nodes 2, ... n
        ...
        edge1[e] = intnod
        edge2[e] = ternod
        #print("Terminal node to be added: ", ternod)
        #print("Edge choosen in utl: ", ichose)
        #print("Resulting edge1: ", edge1)
        #print("Resulting edge2: ", edge2)
    return edge1, edge2
...
def rtl_tree(n, seed=42):
    ...
    root_ind = 2 * n - 2
    # choose the edge on which the root is created
    split = np.random.randint(0, 2 * n - 3)
    
    edge1_new = np.zeros(root_ind, dtype=int)
    edge2_new = np.zeros(root_ind, dtype=int)
    ...
    #print("Split: ", split)
    #print("Rooted edge1: ", edge1_new)
    #print("Rooted edge2: ", edge2_new)
...
def edges_to_tree(classes, edges):
    ...
    for i in range(2 * n - 1):
        node = Node(i)
        if i < n:
            # create all the nodes corresponding to one class
            # the node.label corresponds to the class contained
            node.label = classes[i]
        nodes.append(node)
    for edge in edges:
        # connect the nodes according to edge both ways
